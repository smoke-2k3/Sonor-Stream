...
#ifndef SONOR_STREAM_LOCKFREEQUEUE_H
#define SONOR_STREAM_LOCKFREEQUEUE_H
...
public:
    ...
    /**
     * Bulk pop operation to retrieve multiple elements from the front of the queue.
     *
     * @param[out] outputArray - The output array to store the popped elements
     * @param numElements - The number of elements to pop
     * @return true if elements were popped successfully, false if the queue is empty
     */
    bool bulkPop(T* outputArray, size_t numElements) {
     ...
     */
    __attribute__((unused)) bool peek(T &item) const {
        if (isEmpty()){
            return false;
        } else {
    ...
    INDEX_TYPE size() const {
         ...
         * be positive. Simple.
         *
         * But when writeCounter is equal to INDEX_TYPE_MAX (e.g. U_INT32_MAX) the next push will
         ...
         *
         * e.g. if write is 0, read is 150 and the INDEX_TYPE is u_int8_t where the max value is
...
#endif //SONOR_STREAM_LOCKFREEQUEUE_H